...
let ``Visit record definition test`` () =
    ...
    | Some [ SynFieldOrSpread.SynField (SynField (idOpt = Some id1)); SynFieldOrSpread.SynField (SynField (idOpt = Some id2)) ] when id1.idText = "A" && id2.idText = "B" -> ()
    ...
    let visitor =
        { new SyntaxVisitorBase<_>() with
            member x.VisitRecordDefn(path, fields, range) =
                fields
                |> List.choose (function SynFieldOrSpread.SynField (SynField(idOpt = idOpt)) -> idOpt |> Option.map (fun ident -> ident.idText) | _ -> None)
                |> String.concat ","
                |> Some
        }
