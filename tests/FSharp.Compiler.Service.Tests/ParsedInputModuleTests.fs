...
let ``tryPick record definition test`` () =
    ...
    let fields =
        ...
        ||> ParsedInput.tryPick (fun _path node ->
            match node with
            | SyntaxNode.SynTypeDefn(SynTypeDefn(typeRepr = SynTypeDefnRepr.Simple(SynTypeDefnSimpleRepr.Record(recordFieldsOrSpreads = SynFields fields), _))) -> Some fields
            | _ -> None)
...
"""
    ...
    let ident =
        ...
        ||> ParsedInput.tryPick (fun _path node ->
            match node with
            | SyntaxNode.SynTypeDefnSig(SynTypeDefnSig(typeRepr = SynTypeDefnSigRepr.Simple(SynTypeDefnSimpleRepr.Record(recordFieldsOrSpreads = SynFields fields), _))) ->
                fields
