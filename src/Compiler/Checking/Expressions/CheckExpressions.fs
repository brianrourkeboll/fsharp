            ...
            let tcSpread expr m =
                ...
                if isRecdTy g tyOfSpreadSrcExpr || isAnonRecdTy g tyOfSpreadSrcExpr then
                    let spreadSrcAddrExpr, spreadSrcExpr =
                        ...
                        let wrap, oldAddr, _readonly, _writeonly = mkExprAddrOfExpr g srcTyIsStruct false NeverMutates spreadSrcAddrExpr None m
                        oldAddr, (fun expr -> wrap (mkCompGenLet m spreadSrcAddrVal spreadSrcExpr expr))
...
and TcCopyAndUpdateAnonRecdExpr cenv (overallTy: TType) env tpenv (isStruct, (origExpr, blockSeparator), unsortedFieldIdsAndSynExprsGiven, mWholeExpr) =
    ...
    let spreadSrcs, _fieldIdsInAlphabeticalOrder, fieldTysInAlphabeticalOrder, fieldsInSrcOrder =
        let checkSpreadsLanguageFeature m = checkLanguageFeatureAndRecover g.langVersion LanguageFeature.RecordSpreads m
    ...
    let unsortedIdAndExprsAll =
        [|
            for id, _, tcField in fieldsInSrcOrder do
                yield (id, Choice1Of2 tcField)
    ...
    let anonInfo, sortedFieldTysAll =
        ...
        let sortedFieldTysAll =
            match unsortedIdAndExprsAll.Length - fieldTysInAlphabeticalOrder.Length with
            | 0 -> fieldTysInAlphabeticalOrder
            | diff -> fieldTysInAlphabeticalOrder @ [for _ in 1..diff -> NewInferenceType g]
